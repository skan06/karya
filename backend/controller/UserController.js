...
exports.loginUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(
      new ErrorHandler("please enter your email and your password", 400)
    );
  }
...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    await sendMail({
      email: user.email,
      subject: `Karya Password Recovery`,
      message,
    });
...
// get all singel user details Admin
exports.getSingleUser = catchAsyncErrors(async (req, res, next) => {
  const user = await User.findById(req.params.id);
  if (!user) {
    return next(new ErrorHandler("user is not find with this id", 400));
  }
